...
function DebateChallenge() {
  ...
  useEffect(() => {
    axios.get("http://localhost:8000/api/challenges/")
      .then(res => {
        const match = res.data.find((c) => c.title.toLowerCase().includes("debate-challenge"));
  ...
  const handleStart = () => {
    if (challenge) {
      axios.post("http://localhost:8000/api/challenges/attempt/", {
        ...
        router("/challenges/debate-challenge");
      });
    }
  };
  ...
  return (
    <div className="bg-[#231246] border border-[#39246c] rounded-[16px] shadow-sm p-6">
      <h5 className="text-xl font-bold text-white">DebateChallenge</h5>
