...
import { Material, materialService } from '@/lib/api/services/materialService';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';

interface MaterialWithContent extends Material {
  content: string;
}
...
const EditMaterials = () => {
  ...
  const { data: apiMaterials, isLoading, error } = useQuery({
    queryKey: ['materials'],
    queryFn: materialService.getAll
  });
  
  const mockMaterials: MaterialWithContent[] = [
    {
      id: 1,
  ...
  const materials = error ? mockMaterials : (apiMaterials as MaterialWithContent[] || mockMaterials);
  
  const [editingMaterial, setEditingMaterial] = useState<MaterialWithContent | null>(null);
  const [viewingMaterial, setViewingMaterial] = useState<MaterialWithContent | null>(null);

  const updateMutation = useMutation({
    mutationFn: (updatedMaterial: MaterialWithContent) => {
      const { id, title, type, category, level } = updatedMaterial;
      return materialService.update(id, { title, type, category, level });
    },
    onSuccess: () => {
  ...
  const setMaterials = (newMaterials: MaterialWithContent[]) => {
  };
                      ...
                      <TableCell className="text-right">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem onClick={() => setEditingMaterial(material as MaterialWithContent)}>
                              <Edit className="h-4 w-4 mr-2" /> Edit
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => setViewingMaterial(material as MaterialWithContent)}>
