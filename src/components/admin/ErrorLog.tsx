...
// IMPORT SERVICE & TYPES + REACT-QUERY
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import { errorLogService, type ErrorLog as ErrorLogType } from '@/lib/api/services/errorLogService';
...
const ErrorLog = () => {
  ...
  const [selectedError, setSelectedError] = useState<ErrorLogType | null>(null);

  const queryClient = useQueryClient();

  // Fetch error logs from API via service
  const {
    data: errorLogs = [],
    isLoading,
    isError,
    ...
    queryKey: ['errorLogs'],
    queryFn: () => errorLogService.getAll().then(res => res), // getAll trả về Promise<ErrorLog[]>
  });

  // Update mutation
  const resolveMutation = useMutation({
    mutationFn: (log: ErrorLogType) =>
      errorLogService.update(log.id, { status: 'Resolved' }),
    onSuccess: () => {
  ...
  // Action: resolve error via API
  const handleResolveError = (id: number) => {
    const log = errorLogs.find((l: ErrorLogType) => l.id === id);
    if (log) {
      resolveMutation.mutate(log);
    }
  ...
  // Action: refresh error logs from API
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow>
                  <TableCell colSpan={6} className="text-center text-red-700 font-medium">
                ...
                <TableRow key={log.id}>
                  <TableCell>
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                      log.type === 'API Error' ? 'bg-red-100 text-red-800'
                        : log.type === 'System Error' ? 'bg-purple-100 text-purple-800'
                        : log.type === 'Feature Error' ? 'bg-blue-100 text-blue-800'
                  ...
                  <TableCell>
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                      log.status === 'Open' ? 'bg-yellow-100 text-yellow-800'
                        ...
                        : log.status === 'Resolved' || log.status === 'Closed' ? 'bg-green-100 text-green-800'
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end space-x-2">
                      ...
                      {log.status !== 'Resolved' && log.status !== 'Closed' &&
                        <Button size="sm" onClick={() => handleResolveError(log.id)} className="bg-violet-600 hover:bg-violet-500">
                          Resolve
                        </Button>}
                    </div>
                  </TableCell>
      ...
      <Dialog open={!!selectedError} onOpenChange={open => !open && setSelectedError(null)}>
        <DialogContent className="max-w-xl">
          ...
          {selectedError && <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                  selectedError.status === 'Open' ? 'bg-yellow-100 text-yellow-800'
                    : selectedError.status === 'In Progress' ? 'bg-blue-100 text-blue-800'
                    : selectedError.status === 'Resolved' || selectedError.status === 'Closed' ? 'bg-green-100 text-green-800'
                    : 'bg-gray-100 text-gray-800'
                }`}>
          ...
          <DialogFooter>
            ...
            {selectedError && selectedError.status !== 'Resolved' && selectedError.status !== 'Closed' &&
              <Button onClick={() => handleResolveError(selectedError.id)} className="bg-blue-600 hover:bg-blue-700">
                Mark as Resolved
              </Button>}
          </DialogFooter>
        </DialogContent>
